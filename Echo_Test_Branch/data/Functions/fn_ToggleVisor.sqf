...
if !OPTRE_Hud_On then {
	if !_brokenBool then { // Not Broken 
	  ...
	  //OPTRE_Hud_OverSheildBar = (getText (configfile >> "CfgOptreHudSchemes" >> _hudStyle >> "overSheildBar"));
		...
		//make sure the "visor on" helmet isn't already being worn.
		if _hel then {
			if (!([(_varHelmetArray select 1), _helmet, true] call BIS_fnc_inString) || (_varHelmetArray select 1) == "") then {
				...
				OPTREB_HUD_HelmetOnClass = _helmet;
			};
		};
